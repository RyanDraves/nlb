...
def main(
    ...
    try:
        # Hacky "num trials" for MDP_LLM and HEURISTIC_LLM since
        # they are invoked once per batch
        for _ in range(1 if monte_carlo is None else monte_carlo):
            for reasoning_effort in reasoning_efforts:
                mdp_manual_planner = mdp_manual.MdpPlanner(num_blocks)
                open_loop_planner = open_loop_llm.OpenLoopPlanner(
                ...
                # Run each policy for the specified number of monte carlo trials
                rng_seeds = [42] if monte_carlo is None else list(range(monte_carlo))
                for rng_seed in rng_seeds:
                    console.rule(
                        f'Reasoning Effort: {reasoning_effort}, Seed: {rng_seed}'
                    )
