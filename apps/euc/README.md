...
Three Bazel packages (third-party deps come from the repo-wide `@crates` hub,
resolved from the single `//:Cargo.toml`; first-party deps are wired in BUILD
files only):
...
- **`shared/` (`euc_shared`)** — pure, deterministic rules engine + wire
  ...
  reproducible; see the 1000-game fuzz in `shared/tests.rs`.
